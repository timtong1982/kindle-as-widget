...
const createAction =<T extends {[key:string]: string}> (actionMap: T, prefix: string): Readonly<T> => {
...
const createReducer = <T>(initialState: T, reducerMap: ReducerHanlderType<T>):(state: T, action: ActionType<unknown>)=>T =>{
...
type InferActionType<T, K extends keyof T> = T[K] extends (...args : infer _P) => ActionType<infer R>? ActionType<R> : never;

const appReducer = createReducer(initialAppState, {
    [appActionTypes.SET_TIME]: (state:AppReducerType, action : InferActionType<typeof appActions, 'setTime'>)=>{
        return {
            ...state,
